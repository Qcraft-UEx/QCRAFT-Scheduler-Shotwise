...
def obtain_machine_aws(machine:str) -> AwsDevice:
    """
    Obtains the machine to execute the circuit.

    Args:
        machine (str): The machine ARN to execute the circuit.        
    
    Returns:
        AwsDevice: The machine to execute the circuit.
    """
    if machine == "local":
        return LocalSimulator()
    else:
        return AwsDevice(machine)
    
def get_available_machines_aws() -> list:
    """
    Gets the available machines to execute the circuit.

    Returns:
        list: The available machines to execute the circuit.
    """
    devices = AwsDevice.get_devices()
    devices_arn = [device.arn for device in devices]
    return devices_arn