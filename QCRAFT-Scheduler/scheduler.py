...
class Scheduler:
    ...
    def __init__(self):
        ...
        self.app.route('/code', methods=['POST'])(self.store_code)
    ...
    def select_policy(self, url:str, num_qubits:int, shots:int, user:int, circuit_name:str, maxDepth:int, provider:str, policy:str, machine:str) -> None:
        """
        ...
        """
        data = {"circuit": url, "num_qubits": num_qubits, "shots": shots, "user": user, "circuit_name": circuit_name, "maxDepth": maxDepth, "provider": provider, "machine": machine}
    ...
    def store_url(self) -> tuple: # TODO instead of "both", use a list of providers as an input
        ...
        if not fragment.startswith('circuit='):
            ...
            available_devices_ibm = self.scheduler_policies.get_available_devices_ibm()
            available_devices_aws = self.scheduler_policies.get_available_devices_aws()
            ...
            for provider in providers: #Iterate through the providers to add the elements to the specific provider queue in case the circuit needs to be executed on multiple providers
                machine = request.json.get('machine', available_devices_ibm[-1] if provider == 'ibm' else available_devices_aws[-1]) #TODO maybe instead of taking the last one, take the one with less queue (must be online)
                if machine not in available_devices_ibm and provider == 'ibm':
                    machine = available_devices_ibm[-1]
                if machine not in available_devices_aws and provider == 'aws':
                    machine = available_devices_aws[-1]
                transpilation_machine = machine
                transpilation_backend = self.executeCircuitIBM.obtain_machine(self.service, transpilation_machine)

                shots = providers[provider]
                if transpilation_machine == 'local':
                    ...
                    try:
                        ...
                        if provider == 'ibm':
                            ...
                            maxDepth = self.executeCircuitIBM.get_transpiled_circuit_depth_ibm(circ, transpilation_backend)
                        elif provider == 'aws':
                            #TODO
                            maxDepth = max(sum(1 for j in circuit['cols'] if i < len(j) and j[i] not in {1, 'Measure'}) for i in range(num_qubits))
                    except:
                ...
                self.select_policy(url, num_qubits, shots, user, url, maxDepth, provider, policy, machine)
    ...
    def store_url_circuit(self) -> tuple:
        ...
        if importIBM:
            machine = request.json.get('machine', available_devices_ibm[-1]) #TODO maybe instead of taking the last one, take the one with less queue (must be online)
            if machine not in available_devices_ibm:
                machine = available_devices_ibm[-1]
            transpilation_machine = machine
            transpilation_backend = self.executeCircuitIBM.obtain_machine(self.service, transpilation_machine)                
            ...
            machine = request.json.get('machine', available_devices_aws[-1]) #TODO maybe instead of taking the last one, take the one with less queue (must be online)
            if machine not in available_devices_aws:
                machine = available_devices_aws[-1]
        ...
        self.select_policy(circuit, num_qubits, shots, user, circuit_name, maxDepth, provider, policy, machine)

        return str(user), 200


    def store_code(self) -> tuple:
        """
        Accepts direct code submission in the request body.
        Processes the circuit code and sends it to the policy service.

        Request Parameters:
            code (str): The circuit code (Qiskit or AWS Braket)
            shots (int): The number of shots to execute the circuit
            policy (str): The policy to execute the circuit. Default is 'time'
            circuit_name (str): Name to identify the circuit. Default is 'direct_submission'
        ...
        if request.json.get('code') is None:
            return "Circuit code must be specified", 400
        if request.json.get('shots') is None:
            return "Shots must be specified", 400

        # Get parameters from request
        circuit = request.json['code']
        shots = request.json['shots']
        policy = request.json.get('policy', 'time')
        circuit_name = request.json.get('circuit_name', 'direct_submission')
        ...
        if importIBM:
            ...
            if machine not in available_devices_ibm:
                machine = available_devices_ibm[-1]
            transpilation_machine = machine
            transpilation_backend = self.executeCircuitIBM.obtain_machine(self.service, transpilation_machine)#Maybe get all machines at the start and get the device instead of taking them here
            # Process Qiskit code (IBM)
            circ = self.executeCircuitIBM.code_to_circuit_ibm(circuit)
            ...
            # Get the circuit name from the code
            file_circuit_name_line = next((line.split('#')[0].strip() for line in lines if '= QuantumCircuit(' in line.split('#')[0]), None)
            file_circuit_name = file_circuit_name_line.split('=')[0].strip() if file_circuit_name_line else None
            ...
            return "Unsupported circuit format. Must be either Qiskit or AWS Braket code.", 400

        # Send circuit to the policy service
        self.select_policy(circuit, num_qubits, shots, user, circuit_name, maxDepth, provider, policy, machine)
